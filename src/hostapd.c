...
//ÓÃ»§Ì¬ÔöŒÓsta_info ¹©ºóÃæÄÚºËÌ¬Ê¹ÓÃ111111111177
...
int hostapd_sta_add(struct hostapd_data *hapd,
	...
	if (hapd->driver == NULL || hapd->driver->sta_add == NULL) {
		//nl80211 driverÇý¶¯²»¿ÉÓÃ,¿ÉÄÜ³öÏÖ¿ÕÖžÕë£¬ÎŽž³Öµ
	...
	/****ÖØÒª: Èç¹ûÄÚºËÖØžŽµÄÌíŒÓsta_infoŸÍ»áµŒÖÂÊÖ»ú²»ÄÜœÓÈë£¬×¢ÒâÕâžöÏžœÚ*****/
	if(session == 0){
		params.set = 0;//ÔöŒÓstation		
	}else
		params.set = 1;//ÉèÖÃstation
	//return 0;
	return hapd->driver->sta_add(hapd->drv_priv, &params);

}

//ŽŽœšÓÃ»§Ì¬µÄsta_info œá¹¹Ìå²¢Ìî³äÈÏÖ€µÄ±êÖŸÎ»
			   ...
			   const u8 *ies, size_t ies_len, int reassoc)
{
	...
	*/
	//žŽÖÆËÙÂÊŒ¯ºÏµœsta_infoœá¹¹ÌåÖÐ
	resp = copy_supp_rates(hapd, sta, &elems);
	if (resp != WLAN_STATUS_SUCCESS)
		return resp;	

	
	//žŽÖÆHTÄÜÁŠ±êÖŸÎ»µœsta_infoœá¹¹ÌåÖÐ
	resp = copy_sta_ht_capab(hapd, sta, elems.ht_capabilities,
				 elems.ht_capabilities_len);
	if (resp != WLAN_STATUS_SUCCESS)
		return resp;
	
	//APÖ§³ÖžßËÙÂÊµ«ÊÇsta²»Ö§³ÖžßËÙÂÊ
...
//œâÎö¹ØÁªÖ¡,²¢œ«²ÎÊýÌîÐŽµœsta_infoÖÐ----lenÊÇÊýŸÝ°üµÄ×Ü³€¶È
u8 *hostapd_handle_assoc(struct hostapd_data *hapd,
	...
	pos = mgmt->u.assoc_req.variable;//ÒÆ¶¯ÖžÕëœâÎöex×Ö¶Î
	sta = ap_get_sta(hapd, mgmt->sa);
	//ÈÏÖ€Ê±ºòŸÍÒòžÃ·ÖÅäÁËÄÚŽæ,Èç¹ûÎª¿ÕŽú±íÃ»ÓÐŸ­¹ýÈÏÖ€
	 ...
	 * is used */
	resp = check_assoc_ies(hapd, sta, pos, left, reassoc);//œâÎö²ÎÊý---ºËÐÄº¯Êý ·µ»ØÖµ0 ²ÅÕýÈ·£¬Èç¹û²»ÕýÈ·Ó°ÏìÖÕ¶ËœÓÈë»òÕßËÙÂÊÒì³£
...
/*	
	...
	//Èç¹û²»Ê¹ÓÃ¶ÌŒäžô£¬ÔòÖØÐÂÉèÖÃbeaconÖ¡
	...
	*/
	...
	if (sta->capability & WLAN_CAPABILITY_SHORT_PREAMBLE)
		...
		sta->flags &= ~WLAN_STA_SHORT_PREAMBLE;
/*		
	//Èç¹û²»Ö§³Ö¶ÌÇ°µŒÂë£¬ÔòÖØÐÂÉèÖÃbeaconÖ¡
	if (!(sta->capability & WLAN_CAPABILITY_SHORT_PREAMBLE) &&
...
void hostapd_handle_assoc_cb(struct hostapd_data *hapd,u8 *addr){
	...
	//ÍšÖªÄÚºËÌ¬ÔöŒÓsta_infoll
	//_hapd->handle_assoc_cb(_hapd,sta);
	//ÏÈÒÆ³ýÄÚºËÖÐ¿ÉÄÜÒÑŸ­ÌíŒÓµÄsta_info£¬
	//±ØÐësta_infoÔÚÓÃ»§Ì¬ÒÑŸ­ŽæÔÚŸÍÉŸ³ýÈç¹û²»ŽæÔÚŸÍÖ±œÓ·µ»Ø
...
int hostapd_driver_init(struct hostapd_iface *iface,nl_recv_callback odin_process_bss_event)
{
	...
	if (hapd->driver == NULL || hapd->driver->global_init == NULL) {
		...
		fprintf(stderr, "hapd->driver || hapd->driver->global_init == NULL\n");
		return -1;
	}
	...
	params.ssid_len = hapd->conf->ssid.ssid_len; //SSID length  
    ...
    hapd->drv_priv = hapd->driver->hapd_init(hapd, &params,odin_process_bss_event);
	os_free(params.bridge);
	if (hapd->drv_priv == NULL) {
		fprintf(stderr, "%s driver initialization failed.",
			   hapd->driver->name);
		hapd->driver = NULL;
		return -1;
	}
...
int generate_beacon(struct hostapd_data *_hapd,u8 *da,u8 *bssid,
					const char *ssid,int ssid_len,int probe,char * packet,int *len){
	...
	ieee802_11_build_ap_params(hapd,da,bssid,ssid,ssid_len,probe,&params);//²úÉúbeaconÖ¡,Í·²¿ºÍEX×Ö¶Î·Ö¿ª
	...
	//packet = (char *)os_malloc(params.head_len + params.tail_len);//Îªbeacon·ÖÅäÄÚŽæ
	//wpa_printf(MSG_ERROR, "generate_beacon start 2\n");	
	if (!packet)
		goto free_params;

	os_memcpy(packet, params.head, params.head_len);//žŽÖÆbeaconµÄÍ·²¿
	os_memcpy(packet+params.head_len, params.tail, params.tail_len);//žŽÖÆbeaconµÄÎ²²¿
	//beacon[beacon_len] = '\0';//œáÊø
		       ...
		       const char *config_fname, int debug)
{
	...
	fprintf(stderr, "Configuration file: %s", config_fname);
	...
	fprintf(stderr, "hostapd_interface_init success\n");
	return iface;
}
...
void hostapd_interface_init_bss(struct hapd_interfaces *interfaces){
	...
	if (interfaces->iface[0] == NULL || interfaces->iface[0]->bss[0] == NULL) {
		...
		fprintf(stderr, "interfaces.iface[0]->bss[0] failed\n");
		return ;
    }
	...
	fprintf(stderr, "hostapd_interface_init_bss success\n");
}
...
void hostapd_inf_init(struct hapd_interfaces *interfaces){
		...
		if (!interfaces->iface[0]) {
			fprintf(stderr, "hostapd_inf_init failed\n");
			goto out;
		}

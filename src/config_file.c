...
#undef ecw2cw
	...
	conf->bss = (struct hostapd_bss_config **)os_calloc(1, sizeof(struct hostapd_bss_config *));
	if (conf->bss == NULL) {
		os_free(conf);
		os_free(bss);
		return NULL;
	...
	bss->radius = (struct hostapd_radius_servers*)os_zalloc(sizeof(*bss->radius));
	if (bss->radius == NULL) {
		os_free(conf->bss);
		os_free(conf);
		os_free(bss);
					 ...
					 const char *fname)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		*pos2 = '\0';
		...
		vlan = (struct hostapd_vlan *)os_zalloc(sizeof(*vlan));
		if (vlan == NULL) {
...
{
	const struct mac_acl_entry *aa = (const struct mac_acl_entry *)a;
	const struct mac_acl_entry *bb = (const struct mac_acl_entry *)b;
				       ...
				       struct mac_acl_entry **acl, int *num)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		newacl = (struct mac_acl_entry *)os_realloc_array(*acl, *num + 1, sizeof(**acl));
		if (newacl == NULL) {
				...
				struct hostapd_radius_server **curr_serv)
{
	...
	nserv = (struct hostapd_radius_server *)os_realloc_array(*server, *num_server + 1, sizeof(*nserv));
	if (nserv == NULL)
		return -1;
				   ...
				   char *val)
{
	...
	if (val[0] == '"') {
		if (len < 2 || val[len - 1] != '"')
			return -1;
		len -= 2;
		wep->key[keyidx] = (u8*)os_malloc(len);
...
static int hostapd_parse_intlist(int **int_list, char *val)
{
	...
	*int_list = NULL;
	...
	list = (int *)os_malloc(sizeof(int) * (count + 2));
	if (list == NULL)
		return -1;
	pos = val;
	count = 0;
...
static int hostapd_config_bss(struct hostapd_config *conf, const char *ifname)
{
	...
	if (*ifname == '\0')
		return -1;
	//DEBUG
	all = (struct hostapd_bss_config **)os_realloc_array(conf->bss, conf->num_bss + 1,
				   ...
				   const char *capab)
{
	...
	if (os_strstr(capab, "[SHORT-GI-40]"))
		...
		//conf->ht_capab |= HT_CAP_INFO_SHORT_GI40MHZ | HT_CAP_INFO_SHORT_GI20MHZ/*nm*/;
			       ...
			       char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		...
		bss->logger_syslog_level = (enum hostapd_logger_level)atoi(pos);
	} else if (os_strcmp(buf, "logger_stdout_level") == 0) {
		bss->logger_stdout_level = (enum hostapd_logger_level)atoi(pos);
	} else if (os_strcmp(buf, "logger_syslog") == 0) {
		bss->logger_syslog = (enum hostapd_logger_level)atoi(pos);
	} else if (os_strcmp(buf, "logger_stdout") == 0) {
		bss->logger_stdout = (enum hostapd_logger_level)atoi(pos);
		...
		bss->macaddr_acl = (enum Macaddr_acl)atoi(pos);
		...
		*/
		...
		os_free(bss->ssid.wpa_psk);
		bss->ssid.wpa_psk = (struct hostapd_wpa_psk*)os_zalloc(sizeof(struct hostapd_wpa_psk));
		...
		bss->wpa_psk_radius = (enum Wpa_psk_radius)atoi(pos);
		...
		if (hostapd_config_ht_capab(conf, pos) < 0) {
			wpa_printf(MSG_ERROR, "sdwn Line %d: invalid ht_capab",
				   line);
			return 1;
		}
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	} else if (os_strcmp(buf, "vendor_elements") == 0 && 0) {
		struct wpabuf *elems;
		size_t len = os_strlen(pos);
		if (len & 0x01) {
			wpa_printf(MSG_ERROR,
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
	...
	conf->driver = wpa_drivers[0];//wpa_driver_nl80211_ops   
	if (conf->driver == NULL) {
	...
	fclose(f);
	//fprintf(stderr, "SDWN111" MACSTR" ",MAC2STR(bss->bssid));
	for (i = 0; i < conf->num_bss; i++)

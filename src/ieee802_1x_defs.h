...
/**
 * 解析80211帧中的扩展字段
 * start:扩展字段的首地址,len:扩展部分的长度,elems:解析结构存放结构体
 */
static  ParseRes ieee802_11_parse_elems(const u8 *start, size_t len,
				struct ieee802_11_elems *elems,
				int show_errors)
{
	size_t left = len;
	...
	//fprintf(stderr,"ieee802_11_parse_elems start\n");
	
	//一般帧收到之后是要去除radioatap和FCS的，不过这在内核RX出完成了
	//在这里left代表剩余要解析字段,不去除FCS会解析出错
	//while (left >= 2) {//QOS
	while (left >= 2){
		u8 id, elen;
		...
		if (elen > left) {
			...
			//fprintf(stderr,"ieee802_11_parse_elems early failed show_errors=%d,time=%d(id=%d elen=%d "
			//		   "left=%lu)\n",
			///		   show_errors,time,id, elen, (unsigned long) left);
			return ParseUnknown;//may wrong for FCS,here is last position of packet
			//return ParseFailed;
		}
		...
		case WLAN_EID_VENDOR_SPECIFIC:
			//fprintf(stderr,"left=%d,elen=%d,time=%d\n",left,elen,time);
